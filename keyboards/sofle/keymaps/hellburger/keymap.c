 ...
 * `-----------------------------------------/       /     \      \-----------------------------------------'
 *            | LCTR | LAlt | WIN  | LOWER| /SPACE  /       \ENTER \  |RAISE |  WIN | RAlt | RCTR |
 *            |      |      |      |      |/       /         \      \ |      |      |      |      |
 *            `----------------------------------'           '------''---------------------------'
 */
  [_QWERTY] = LAYOUT(
...
static void render_logo(void) {
    static const char PROGMEM outline_layer[] = {
        0,254,130,130,130,130,130,130,130,130,  2,  2,  4,  8, 16, 32, 32, 16,  8,  4,  2,  2,130,130,130,130,130,130,130,130,254,  0,  0,240, 16, 16, 16, 16, 16, 16, 16, 16, 17, 22, 24,  0,  0,  0,  0,  0,  0, 24, 22, 17, 16, 16, 16, 16, 16, 16, 16, 16,240,  0,  0, 15,136, 72, 40, 24,  8,  0,  0,  0,  0,128,192, 32, 16,  8,  8, 16, 32,192,128,  0,  0,  0,  8,  8, 24, 40, 72,136, 15,  0,  0,127, 64, 64, 64, 64, 64, 64, 64, 64, 64, 65, 65, 65, 65, 65, 65, 65, 65, 65, 65, 64, 64, 64, 64, 64, 64, 64, 64, 64,127,  0,
    };

    static const char PROGMEM inner_layer[] = {
        0,254,130,186,170,170,170,170,170,170, 42, 82,164, 72,144, 32, 32,144, 72,164, 82, 42,170,170,170,170,170,170,186,130,254,  0,  0,240, 16,208, 80, 80, 80, 80, 80, 80, 81, 86, 88, 67, 76, 49, 49, 76, 67, 88, 86, 81, 80, 80, 80, 80, 80, 80,208, 16,240,  0,  0, 15,136, 75, 42, 26,138, 66,162, 82, 42,150,200, 36, 18, 10, 10, 18, 36,200,150, 42, 82,162, 74,138, 26, 42, 75,136, 15,  0,  0,127, 64, 92, 82, 81, 86, 85, 84, 84, 84, 85, 85, 85, 85, 85, 85, 85, 85, 85, 85, 84, 84, 84, 85, 86, 81, 82, 92, 64,127,  0,
    };

    static const char PROGMEM raw_logo_full[] = {
        0,254,130,186,186,186,186,186,186,186, 58,114,228,200,144, 32, 32,144,200,228,114, 58,186,186,186,186,186,186,186,130,254,  0,  0,240, 16,208,208,208,208,208,208,208,209,214,216,195,207,255,255,207,195,216,214,209,208,208,208,208,208,208,208, 16,240,  0,  0, 15,136, 75, 43, 27,139,195,227,115, 59,159,207, 39, 19, 11, 11, 19, 39,207,159, 59,115,227,203,139, 27, 43, 75,136, 15,  0,  0,127, 64, 92, 94, 95, 95, 93, 92, 92, 92, 93, 93, 93, 93, 93, 93, 93, 93, 93, 93, 92, 92, 92, 93, 95, 95, 94, 92, 64,127,  0,
    };

    oled_write_raw_P(outline_layer, sizeof(outline_layer));

    switch (get_highest_layer(layer_state)) {
        // @TODO: Finish cases
        case _QWERTY:
            oled_write_raw_P(inner_layer, sizeof(inner_layer));
            break;
        case _RAISE:
            oled_write_raw_P(raw_logo_full, sizeof(raw_logo_full));
            break;
        case _LOWER:
            oled_write_raw_P(outline_layer, sizeof(outline_layer));
            break;
...
static void print_status_narrow(void) {
    ...
    // Print current mode
    oled_write_P(PSTR("\n\n"), false);
    oled_write_ln_P(PSTR("Seif"), false);
...
bool encoder_update_user(uint8_t index, bool clockwise) {
    if (index == 0) {
        // clockwise seems to be incorrect, unclear if this is a bug in code or wiring
        if (clockwise) {
            tap_code(KC_VOLD);
        } else {
            tap_code(KC_VOLU);
